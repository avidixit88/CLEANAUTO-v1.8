...
def compute_scalp_signal(
    ...
    *,
    ...
    if volume_gate_active:
        reversal_structure_ok = bool(liquidity_event or micro_structure_event or rsi_event or macd_event or orb_event)
        ...
        if favorable_zone_ok and reversal_structure_ok:
            ...
            volume_missing_penalty = int(round(5.0 * float(liquidity_mult))) if isinstance(liquidity_mult, (int, float)) else 5
            if str(bias).upper() == "LONG":
...
def compute_ride_signal(
    ...
    **_ignored: object,
    ...
    # accept line priority follows the strongest legitimate impulse source.
    if direction == "LONG":
        if impulse_type_hint == "VWAP" and ref_vwap is not None:
            accept_line, accept_src = float(ref_vwap), "VWAP"
        elif impulse_type_hint == "ORB" and orb_high is not None:
            accept_line, accept_src = float(orb_high), "ORB"
        elif impulse_type_hint == "PIVOT" and swing_hi is not None:
            accept_line, accept_src = float(swing_hi), "PIVOT"
        elif vwap_reclaim and ref_vwap is not None:
            accept_line, accept_src = float(ref_vwap), "VWAP"
        ...
        if impulse_type_hint == "VWAP" and ref_vwap is not None:
            accept_line, accept_src = float(ref_vwap), "VWAP"
        elif impulse_type_hint == "ORB" and orb_low is not None:
            accept_line, accept_src = float(orb_low), "ORB"
        elif impulse_type_hint == "PIVOT" and swing_lo is not None:
            accept_line, accept_src = float(swing_lo), "PIVOT"
        elif vwap_reject and ref_vwap is not None:
            accept_line, accept_src = float(ref_vwap), "VWAP"
    ...
    #
    # Additional refinement: instead of widening/narrowing by PRE vs CONFIRMED, use the current
    # RIDE score itself as the conviction signal. High-conviction (score >= 90) setups get
    # tighter/shallower pullback bands; everything else gets slightly deeper bands.
    #
    ...
    high_conviction = bool(float(score or 0.0) >= 87.0)
    width_bias_inner = 0.00 if high_conviction else 0.03
    width_bias_outer = 0.00 if high_conviction else 0.05
    pb_inner_mult = float(np.clip(0.18 + 0.14 * q_weak + width_bias_inner, 0.15, 0.35))
    pb_outer_mult = float(np.clip(0.42 + 0.30 * q_weak + width_bias_outer, 0.35, 0.80))
    pb_inner = float(pb_inner_mult * float(atr_last))
    pb_outer = float(pb_outer_mult * float(atr_last))
    ...
    if direction == "LONG":
        if impulse_idx is not None:
            break_trigger = float(max(float(impulse_level), float(df["high"].iloc[impulse_idx])))
        else:
            break_trigger = float(impulse_level)
        ...
        if impulse_idx is not None:
            break_trigger = float(min(float(impulse_level), float(df["low"].iloc[impulse_idx])))
        else:
            break_trigger = float(impulse_level)
        pb1 = float(accept_line) - pb_inner
        pb2 = float(accept_line) + pb_outer
    ...
    # even after a legitimate impulse, only allow a breakout entry if price is actually
    # being accepted beyond the impulse source. This reduces paying up into one-bar spikes.
    breakout_ref = None
    if direction == "LONG":
        if impulse_type_hint == "ORB" and orb_high is not None:
            breakout_ref = float(orb_high)
        elif impulse_type_hint == "VWAP" and ref_vwap is not None:
            breakout_ref = float(ref_vwap)
        elif impulse_type_hint == "PIVOT" and swing_hi is not None:
            breakout_ref = float(swing_hi)
    else:
        if impulse_type_hint == "ORB" and orb_low is not None:
            breakout_ref = float(orb_low)
        elif impulse_type_hint == "VWAP" and ref_vwap is not None:
            breakout_ref = float(ref_vwap)
        elif impulse_type_hint == "PIVOT" and swing_lo is not None:
            breakout_ref = float(swing_lo)

    recent_break_closes = df["close"].astype(float).iloc[-int(min(2, len(df))):]
    if breakout_ref is not None and len(recent_break_closes):
        if direction == "LONG":
            breakout_acceptance_ok = bool((recent_break_closes >= float(breakout_ref) - buffer).any() and float(recent_break_closes.iloc[-1]) >= float(breakout_ref) - buffer)
        else:
            breakout_acceptance_ok = bool((recent_break_closes <= float(breakout_ref) + buffer).any() and float(recent_break_closes.iloc[-1]) <= float(breakout_ref) + buffer)
    else:
        breakout_acceptance_ok = bool(accept_ok)

    if actionable:
        # Pullback-first selection for volatile low-priced names. Breakout should be rare
        # and only chosen when the move is elite, not too extended, and clearly a better
        # immediate execution than the pullback band.
        ext_above_accept = max(0.0, float(last_price - accept_line)) if direction == "LONG" else max(0.0, float(accept_line - last_price))
        ext_above_pullback = max(0.0, float(last_price - pb1)) if direction == "LONG" else max(0.0, float(pb1 - last_price))
        breakout_extension_ok = bool(ext_above_accept <= 0.60 * float(atr_last) and ext_above_pullback <= 0.35 * float(atr_last))
        breakout_elite_pre = bool(stage == "PRE" and (impulse_type_hint in ("ORB", "VWAP")) and impulse_legitimacy >= 0.86 and accept_ok and breakout_acceptance_ok and breakout_extension_ok)
        breakout_confirmed_ok = bool(stage == "CONFIRMED" and (impulse_type_hint in ("ORB", "VWAP")) and impulse_legitimacy >= 0.72 and breakout_acceptance_ok and breakout_extension_ok)
        breakout_margin_ok = bool(dist_br <= (0.75 * max(dist_pb_band, 1e-9)))
        breakout_bias = bool((breakout_confirmed_ok or breakout_elite_pre) and near_break and not stale_breakout and breakout_margin_ok)
        pullback_bias = bool(near_pullback and (stage == "PRE" or accept_ok or impulse_legitimacy < 0.86))

        if near_pullback and not near_break:
            choose_pullback = True
        elif near_break and not near_pullback:
            choose_pullback = bool(not breakout_bias)
        elif near_pullback and near_break:
            # When both are viable, default to pullback unless breakout is materially better.
            choose_pullback = bool(not breakout_bias)
        else:
            choose_pullback = bool(near_pullback)
    ...
    if bool(ride_zone_ctx.get("hostile")):
        ride_zone_adj -= 6.0 + 4.0 * host_q
        if str(entry_mode or '').upper() == 'BREAKOUT':
            ride_zone_adj -= (2.0 + 2.0 * host_q)
    score = _cap_score(pts + ride_zone_adj)
