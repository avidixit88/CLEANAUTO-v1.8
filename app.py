...
with col_cd1:
    if st.button("Clear cooldowns", use_container_width=True, key="ui_clear_cooldowns"):
        st.session_state.last_alert_ts = {}
        st.sidebar.success("Cooldowns cleared")
with col_cd2:
    if st.button("Clear signal state", use_container_width=True, key="ui_clear_signal_state"):
        st.session_state.symbol_state = {}
        st.session_state.pending_confirm = {}
...
refresh_seconds = st.sidebar.slider("Refresh every (seconds)", 10, 180, 45, 5, key="ui_refresh_seconds") if auto_refresh else None
